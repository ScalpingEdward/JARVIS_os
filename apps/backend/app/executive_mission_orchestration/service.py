...
class ExecutiveMissionOrchestrationService:
    ...
    def create(self, payload: OrchestrationCreate) -> OrchestrationRecord:
        ...
        with self._lock:
            duplicate = any(
                item.workspace_id == payload.workspace_id and item.title == payload.title
            ...
            self._write_audit(
                ...
                payload.owner_id,
                record.id,
                {"missions": len(payload.missions), "agents": len(payload.agents)},
            )
        return record
    ...
    def _mission_score(mission, agents) -> MissionScore:
        ...
        score = (
            _PRIORITY_WEIGHT[mission.priority] * 0.3
            + mission.strategic_value * 0.3
            + mission.urgency * 0.25
            + readiness * 0.25
            - risk_penalty
        )
        return MissionScore(
            ...
            explanation=[
                f"Priority tier contribution: {_PRIORITY_WEIGHT[mission.priority]:.1f}",
                f"Strategic value: {mission.strategic_value:.1f}",
                f"Urgency: {mission.urgency:.1f}",
                f"Capability readiness: {readiness:.1f}%",
    ...
    def _choose_agent(task, agents, remaining_hours: dict[str, float]) -> AgentAssignment:
        ...
        for agent in agents:
            ...
            capacity_fit = min(1.0, remaining / task.duration_hours) if task.duration_hours else 1.0
        ...
        if not eligible:
            return AgentAssignment(
                ...
                agent_id=None,
                fit_score=0,
                explanation=["No candidate agent matched the task constraints"],
            )
        ...
        if capability_fit < 1.0 or remaining < task.duration_hours:
            return AgentAssignment(
                ...
                explanation=["No agent has both full capability fit and sufficient remaining capacity"],
            )
        ...
        return AgentAssignment(
            ...
            explanation=[
                ...
                f"Remaining capacity before assignment: {remaining:.1f}h",
            ],
        )
    ...
    def analyze(self, orchestration_id: UUID, workspace_id: str, actor_id: str) -> OrchestrationRecord:
        with self._lock:
            ...
            ranked = sorted(
                (self._mission_score(mission, record.agents) for mission in record.missions),
                key=lambda item: (-item.priority_score, item.mission_key),
            )
            mission_rank = {item.mission_key: index for index, item in enumerate(ranked)}
            mission_map = {mission.key: mission for mission in record.missions}
            remaining_hours = {
                agent.agent_id: max(0.0, agent.available_hours - agent.current_load_hours)
            ...
            for score in ranked:
                ...
                for task_key in self._topological_order(mission.tasks):
                    ...
                    assignment = self._choose_agent(task, record.agents, remaining_hours)
                    assignment = assignment.model_copy(update={"mission_key": mission.key})
                    ...
                    dependency_end = max(
                        (end_times.get(f"{mission.key}:{dependency}", 0.0) for dependency in task.dependency_keys),
                        default=0.0,
                    )
                ...
                if capacity > 0 and used / capacity >= 0.9:
                    conflicts.append(
                        ResourceConflict(
                            ...
                            severity="high" if used > capacity else "warning",
                            explanation=f"Agent utilization is {used / capacity * 100:.1f}%",
                        )
                    )

            utilization = {}
            for agent_id, initial in initial_hours.items():
                used = initial - remaining_hours.get(agent_id, 0.0)
                utilization[agent_id] = round((used / initial * 100) if initial else 0.0, 2)
            ...
            if duration > record.planning_horizon_hours:
                recommendations.append("Split the mission portfolio into governed execution waves")
            ...
            analysis = OrchestrationAnalysis(
                ...
                conflicts=conflicts,
                projected_duration_hours=round(duration, 2),
                horizon_fit=duration <= record.planning_horizon_hours and not deferred,
            ...
            self._write_audit(
                ...
                actor_id,
                orchestration_id,
                {"deferred_tasks": len(deferred), "conflicts": len(conflicts)},
            )
            return updated
    ...
    def approve(self, orchestration_id: UUID, payload: ApprovalRequest) -> OrchestrationRecord:
        with self._lock:
            ...
            self._write_audit(
                payload.workspace_id,
                f"orchestration-{payload.decision.value}d",
                payload.reviewer_id,
                orchestration_id,
    ...
    def status(self, workspace_id: str) -> OrchestrationStatusResponse:
        ...
        return OrchestrationStatusResponse(
            ...
            pending_approval=sum(item.status == OrchestrationStatus.pending_approval for item in records),
            approved=sum(item.status == OrchestrationStatus.approved for item in records),
            rejected=sum(item.status == OrchestrationStatus.rejected for item in records),
            conflicts=sum(len(item.analysis.conflicts) for item in records if item.analysis),
        )
