...
def evaluate_recovery_authorization(req: ReconciliationAuthorizationRequest, authorize: bool = False) -> ReconciliationAuthorizationDecision:
    ...
    steps = sorted(req.recovery_steps, key=lambda x: x.order)
    ...
    blocking = {'risk-brain-hard-block','invalid-source-admission','duplicate-source','no-affected-consumers','healthy-consumer-overlap','non-contiguous-recovery-order','recovery-sequence-coverage-mismatch','healthy-consumer-targeted','out-of-order-step-approval'}
