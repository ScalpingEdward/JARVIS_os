...
class RecoveryReliabilityReceiptReconciliationGovernance:
    """Reconcile exact fresh recovery evidence without performing runtime mutation."""

    def __init__(self, *, min_recovery_score: float = 0.8, max_evidence_age_seconds: int = 900) -> None:
    ...
    def reconcile(self, record: RecoveryCompletionRecord, *, source_state: str, source_human_approved: bool) -> RecoveryCompletionRecord:
        ...
        if len(receipt_consumers) != len(set(receipt_consumers)) or len(nonces) != len(set(nonces)):
            invalid = True
        if any(not nonce or nonce in self.nonces for nonce in nonces):
            invalid = True
        ...
        for receipt in record.receipts:
            exact = (
                ...
                and receipt.recovery_score >= self.min_recovery_score
                and 0 <= receipt.evidence_age_seconds <= self.max_evidence_age_seconds
            )
            matching += int(exact)
