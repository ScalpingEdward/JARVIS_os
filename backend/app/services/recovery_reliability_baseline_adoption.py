...
class RecoveryReliabilityBaselineAdoptionGovernance:
    """Authorize exact consumer adoption and verify fresh receipts; never mutates runtime consumers."""
    ...
    def create(self, record: RecoveryReliabilityAdoptionRecord, *, source_state: str, source_human_approved: bool) -> RecoveryReliabilityAdoptionRecord:
        ...
        invalid = (
            ...
            or key in self.source_consumers
        )
        if invalid:
    ...
    def require_receipt(self, record_id: str) -> RecoveryReliabilityAdoptionRecord:
        record = self.records[record_id]
        if record.state != "authorized" or record.risk_brain_blocked:
            record.state = "blocked"
        else:
            record.state = "receipt-required"
        self._audit(record, "receipt-required")
        return record

    def verify_receipt(self, record_id: str, receipt: AdoptionReceipt) -> RecoveryReliabilityAdoptionRecord:
        record = self.records[record_id]
        exact = (
            record.state == "receipt-required"
        ...
        if not exact:
            record.state = "blocked"
            self._audit(record, "receipt-rejected")
            return record
