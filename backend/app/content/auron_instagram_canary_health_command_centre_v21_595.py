...
class InstagramCanaryControlError(RuntimeError):
    pass


@dataclass(frozen=True)
class InstagramHealthEvidence:
    evidence_id: str
    provider_id: str
    adapter_id: str
...
class InstagramCanaryHealthCommandCentre:
    """G8 provider health/drift certification plus governed Instagram canary controls."""

    def __init__(self, db_path: str | Path, adapter: InstagramDraftPreviewCanaryAdapter,
    ...
    def _init_schema(self):
        with self._connect() as c:
            c.executescript('''
                CREATE TABLE IF NOT EXISTS instagram_canary_health_evidence(
                ...
                CREATE TABLE IF NOT EXISTS instagram_canary_command_journal(
    ...
    def fingerprint(self) -> str: return self._hash(asdict(self.adapter.descriptor()))

    def record_health(self, *, healthy: bool, observed_at: str | None=None) -> InstagramHealthEvidence:
        d=self.adapter.descriptor(); ts=observed_at or self._now(); fp=self.fingerprint()
        eid='instagram-health-'+self._hash({'provider':d.provider_id,'adapter':d.adapter_id,'fp':fp,'healthy':healthy,'at':ts})[:24]
        with self._connect() as c:
            c.execute('INSERT OR IGNORE INTO instagram_canary_health_evidence VALUES (?,?,?,?,?,?)',
                (eid,d.provider_id,d.adapter_id,fp,int(healthy),ts))
        return InstagramHealthEvidence(eid,d.provider_id,d.adapter_id,fp,healthy,ts)

    def certify_latest_health(self, *, now: str | None=None) -> dict:
        with self._connect() as c:
            row=c.execute('SELECT * FROM instagram_canary_health_evidence ORDER BY observed_at DESC,evidence_id DESC LIMIT 1').fetchone()
        if row is None:
    ...
    def snapshot(self) -> dict:
        ...
        actions=self._rows(self.adapter.db_path,'SELECT provider_ref,vertical,provider_id,scope,action_key,payload_hash,state,created_at FROM instagram_draft_preview_canary_actions ORDER BY created_at DESC,provider_ref')
        stops=self._rows(self.adapter.db_path,'SELECT * FROM instagram_draft_preview_canary_stops ORDER BY stopped_at DESC,activation_id')
        alerts=[]
        ...
        return {'workspace':'instagram-draft-preview-canary','descriptor':asdict(self.adapter.descriptor()),'health':health,
            'actions':actions,'executions':executions,'reconciliations':reconciliations,'stops':stops,'alerts':tuple(alerts),
            'public_publish_enabled':False,'provider_write_enabled':False,'network_transport_enabled':False,
            'production_transport_enabled':False,'commands_execute_directly':False}

    def stop(self, activation_id: str, *, actor: str, reason: str='operator-stop') -> dict:
        if not actor.strip(): raise InstagramCanaryControlError('actor required')
        self.adapter.stop_canary(activation_id=activation_id,reason=reason)
        return {'activation_id':activation_id,'state':'stopped','public_publish_enabled':False,'production_transport_enabled':False}

    def record_command(self, command_text: str, *, actor: str) -> dict:
        command=command_text.strip(); operator=actor.strip()
        if not command or not operator: raise InstagramCanaryControlError('command and actor required')
        with self._connect() as c:
            cur=c.execute('INSERT INTO instagram_canary_command_journal(actor,command_text,state,created_at) VALUES (?,?,?,?)',
                (operator,command,'recorded-not-executed',self._now()))
        return {'command_id':int(cur.lastrowid),'actor':operator,'command_text':command,'state':'recorded-not-executed'}
