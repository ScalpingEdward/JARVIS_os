"""API contract for PHOENIX v21.159 coordinated recovery completion governance."""
from fastapi import APIRouter, HTTPException
from pydantic import BaseModel, Field

from app.services.re_adoption_reconciliation import AdoptionReceipt, ReAdoptionReconciliationService

router = APIRouter(prefix="/v1/re-adoption-reconciliation", tags=["re-adoption-reconciliation"])
service = ReAdoptionReconciliationService()
...
def create_record(payload: CreateIn) -> dict:
    try:
        receipts = [AdoptionReceipt(**item.model_dump()) for item in payload.receipts]
...
def record(record_id: str) -> dict:
    try:
        ...
        raise HTTPException(status_code=404, detail="reconciliation record not found") from exc


@router.post("/records/{record_id}/actions")
def action(record_id: str, payload: ActionIn) -> dict:
    if payload.action != "approve-completion":
