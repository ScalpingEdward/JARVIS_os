...
router = APIRouter(
    prefix='/auron/demo1/v21.396',
    tags=['auron-demo1-telegram-successor-next-generation-six-recertification'],
)
...
_VALIDATE_PHRASE = 'VALIDATE AURON TELEGRAM SUCCESSOR NEXT GENERATION SIX DRIFT REMEDIATION'
_RECERTIFY_PHRASE = 'RECERTIFY AURON TELEGRAM SUCCESSOR NEXT GENERATION SIX SUCCESSION'
_RENEW_PHRASE = 'RENEW AURON TELEGRAM SUCCESSOR NEXT GENERATION SIX ASSURANCE BASELINE'
...
class RenewedBaselineRequest(BaseModel):
    ...
    renewed_successor_next_generation_six_hash: str = Field(min_length=64, max_length=64, pattern='^[0-9a-f]{64}$')
...
def reset_telegram_successor_next_generation_six_recertification_store() -> None:
...
def validate_remediation(payload: RemediationValidationRequest) -> dict:
    if payload.validation_phrase != _VALIDATE_PHRASE:
        raise HTTPException(status_code=403, detail='Explicit successor-next-generation-six remediation validation required')
    existing = _validation_store.get(payload.drift_id)
    if existing is not None:
        return {'state': 'telegram-successor-next-generation-six-remediation-already-validated', 'validation': existing, 'idempotent_replay': True, 'external_calls_made': 0}
    ...
    if monitoring is None or drift is None or resolution is None:
        raise HTTPException(status_code=409, detail='Resolved v21.395 drift evidence required')
    ...
    expected_hash = monitoring.get('active_successor_next_generation_six_hash')
    checks = {
        'drift_resolved': drift.get('drift_state') == 'successor-next-generation-six-drift-resolved',
        ...
        'monitoring_requires_recertification': monitoring.get('monitoring_state') == 'successor-next-generation-six-drift-resolved-recertification-required',
    }
    ...
    validation = {
        'validation_id': str(uuid4()), **data,
        'validation_state': 'successor-next-generation-six-remediation-validated',
    ...
    return {'state': 'telegram-successor-next-generation-six-remediation-validated', 'validation': validation, 'external_calls_made': 0}
...
def recertify_succession(payload: SuccessionRecertificationRequest) -> dict:
    if payload.recertification_phrase != _RECERTIFY_PHRASE:
        raise HTTPException(status_code=403, detail='Explicit successor-next-generation-six succession recertification required')
    ...
    if existing is not None:
        return {'state': 'telegram-successor-next-generation-six-succession-already-recertified', 'recertification': existing, 'idempotent_replay': True, 'external_calls_made': 0}
    if validation.get('validation_state') != 'successor-next-generation-six-remediation-validated' or validation.get('immutable') is not True:
    ...
    recertification = {
        'recertification_id': str(uuid4()), **data,
        'recertification_state': 'successor-next-generation-six-succession-recertified',
        'integrity_hash': _hash(data), 'immutable': True,
    ...
    return {'state': 'telegram-successor-next-generation-six-succession-recertified', 'recertification': recertification, 'external_calls_made': 0}


@router.post('/baseline/renew')
def renew_baseline(payload: RenewedBaselineRequest) -> dict:
    if payload.renewal_phrase != _RENEW_PHRASE:
        raise HTTPException(status_code=403, detail='Explicit successor-next-generation-six assurance baseline renewal required')
    ...
    if existing is not None:
        return {'state': 'telegram-successor-next-generation-six-baseline-already-renewed', 'baseline': existing, 'idempotent_replay': True, 'external_calls_made': 0}
    ...
    superseded_hash = monitoring['active_successor_next_generation_six_hash']
    if payload.renewed_successor_next_generation_six_hash == superseded_hash:
    ...
    data = {
        ...
        'superseded_successor_next_generation_six_hash': superseded_hash,
        'renewed_successor_next_generation_six_hash': payload.renewed_successor_next_generation_six_hash,
    ...
    baseline = {
        ...
        'baseline_state': 'renewed-successor-next-generation-six-baseline-active',
        'integrity_hash': _hash(data), 'immutable': True,
    ...
    monitoring.update(
        active_successor_next_generation_six_hash=payload.renewed_successor_next_generation_six_hash,
        monitoring_state='renewed-successor-next-generation-six-monitoring-active',
        next_audit_due_at=data['next_audit_due_at'],
        open_drift_id=None,
    )
    return {
        'state': 'telegram-renewed-successor-next-generation-six-baseline-active',
        'baseline': baseline,
        'monitoring': monitoring,
        'external_calls_made': 0,
        'next_layer': 'renewed-successor-next-generation-six-continuity-health-expiry-governance',
    }
