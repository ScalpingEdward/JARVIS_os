...
router = APIRouter(
    prefix='/auron/demo1/v21.393',
    tags=['auron-demo1-telegram-expired-renewed-successor-next-generation-five-restoration'],
)
...
_ADMIT_PHRASE = 'ADMIT AURON TELEGRAM EXPIRED RENEWED SUCCESSOR NEXT GENERATION FIVE RECERTIFICATION'
_RESTORE_PHRASE = 'RESTORE AURON TELEGRAM RENEWED SUCCESSOR NEXT GENERATION FIVE CONTINUITY'
_ESTABLISH_PHRASE = 'ESTABLISH AURON TELEGRAM SUCCESSOR NEXT GENERATION SIX BASELINE'
...
class SuccessorBaselineRequest(BaseModel):
    ...
    successor_next_generation_six_hash: str = Field(min_length=64, max_length=64, pattern='^[0-9a-f]{64}$')
...
def reset_telegram_expired_renewed_successor_next_generation_five_restoration_store() -> None:
...
def admit_recertification(payload: RecertificationAdmissionRequest) -> dict:
    if payload.admission_phrase != _ADMIT_PHRASE:
        raise HTTPException(status_code=403, detail='Explicit expired renewed successor-next-generation-five recertification admission required')
    existing = _admission_store.get(payload.continuity_id)
    if existing is not None:
        return {'state': 'telegram-expired-renewed-successor-next-generation-five-recertification-already-admitted', 'admission': existing, 'idempotent_replay': True, 'external_calls_made': 0}
    ...
    if continuity is None or expiry is None:
        raise HTTPException(status_code=409, detail='Expired v21.392 baseline with immutable expiry evidence required')
    checks = {
        'continuity_expired': continuity.get('continuity_state') == 'renewed-successor-next-generation-five-baseline-expired-recertification-required',
        'expiry_state_valid': expiry.get('expiry_state') == 'renewed-successor-next-generation-five-baseline-expired',
    ...
    admission = {
        ...
        **data,
        'admission_state': 'expired-renewed-successor-next-generation-five-recertification-admitted',
    ...
    return {'state': 'telegram-expired-renewed-successor-next-generation-five-recertification-admitted', 'admission': admission, 'external_calls_made': 0}
...
def restore_continuity(payload: ContinuityRestorationRequest) -> dict:
    if payload.restoration_phrase != _RESTORE_PHRASE:
        raise HTTPException(status_code=403, detail='Explicit renewed successor-next-generation-five continuity restoration required')
    existing = _restoration_store.get(payload.continuity_id)
    if existing is not None:
        return {'state': 'telegram-renewed-successor-next-generation-five-continuity-already-restored', 'restoration': existing, 'idempotent_replay': True, 'external_calls_made': 0}
    ...
    data = {
        ...
        'expired_baseline_hash': admission['expired_baseline_hash'],
    ...
    restoration = {
        ...
        **data,
        'restoration_state': 'renewed-successor-next-generation-five-continuity-restored-awaiting-successor-next-generation-six-baseline',
        'integrity_hash': _hash(data),
        'immutable': True,
    ...
    return {'state': 'telegram-renewed-successor-next-generation-five-continuity-restored', 'restoration': restoration, 'continuity': continuity, 'external_calls_made': 0}
...
def establish_successor_baseline(payload: SuccessorBaselineRequest) -> dict:
    if payload.establishment_phrase != _ESTABLISH_PHRASE:
        raise HTTPException(status_code=403, detail='Explicit successor-next-generation-six baseline establishment required')
    existing = _succession_store.get(payload.continuity_id)
    if existing is not None:
        return {'state': 'telegram-successor-next-generation-six-baseline-already-established', 'succession': existing, 'idempotent_replay': True, 'external_calls_made': 0}
    ...
    checks = {
        ...
        'successor_hash_distinct': payload.successor_next_generation_six_hash != previous_hash,
    }
    blockers = [name for name, passed in checks.items() if not passed]
    if blockers:
    ...
    data = {
        ...
        'successor_next_generation_six_hash': payload.successor_next_generation_six_hash,
    ...
    succession = {
        ...
        **data,
        'succession_state': 'successor-next-generation-six-baseline-active',
        'integrity_hash': _hash(data),
        'immutable': True,
    ...
    continuity.update(
        continuity_state='successor-next-generation-six-baseline-active',
        active_baseline_hash=payload.successor_next_generation_six_hash,
        next_health_check_due_at=data['next_health_check_due_at'],
        valid_until=data['valid_until'],
    )
    baseline = _baseline_store.get(continuity.get('monitoring_id'))
    if baseline is not None:
        baseline['baseline_state'] = 'renewed-successor-next-generation-five-baseline-superseded'
        baseline['superseded_by_succession_id'] = succession['succession_id']
    return {
        'state': 'telegram-successor-next-generation-six-baseline-established',
        'succession': succession,
        'continuity': continuity,
        'superseded_baseline': baseline,
        'external_calls_made': 0,
        'next_layer': 'successor-next-generation-six-stabilization-observation-certification',
    }
...
def status() -> dict:
    return {
        ...
        'successions': len(_succession_store),
        'external_calls_made': 0,
        'mode': 'expired-renewed-successor-next-generation-five-restoration-succession-governance',
    }


@router.get('/command-center', response_class=HTMLResponse)
def command_center() -> str:
    return '<!doctype html><html><head><title>AURON v21.393</title></head><body><h1>AURON TELEGRAM EXPIRED RENEWED SUCCESSOR NEXT GENERATION FIVE RESTORATION COMMAND CENTER</h1><p>Recertification admission, continuity restoration and successor-next-generation-six baseline succession governance.</p><p>Safe mode: no Telegram API call, no provider execution, no outbound message.</p></body></html>'
