...
def _pending_for(req: DialogueRequest) -> list:
    ...
    return [item for item in items if item.requested_by == req.operator_id and item.arguments.get('session_id') == req.session_id]


def _existing(req: DialogueRequest) -> object | None:
...
def _request_approval(req: DialogueRequest, policy: dict) -> object:
...
@router.get('/pending')
def pending(session_id: str = 'approval', workspace_id: str = 'demo', operator_id: str = 'brano') -> dict:
    req = DialogueRequest(session_id=session_id, workspace_id=workspace_id, operator_id=operator_id, command='approval status')
    items = [_serialize(item) for item in _pending_for(req)]
...
def command_center() -> str:
    ...
    html = html.replace('AURON POLICY DECISION LEDGER COMMAND CENTER', 'AURON APPROVAL HANDOFF COMMAND CENTER')
    return html
