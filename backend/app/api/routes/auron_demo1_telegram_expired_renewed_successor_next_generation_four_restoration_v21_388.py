...
router = APIRouter(
    prefix='/auron/demo1/v21.388',
    tags=['auron-demo1-telegram-expired-renewed-successor-next-generation-four-restoration'],
)
...
_ADMIT_PHRASE = 'ADMIT AURON TELEGRAM EXPIRED RENEWED SUCCESSOR NEXT GENERATION FOUR RECERTIFICATION'
_RESTORE_PHRASE = 'RESTORE AURON TELEGRAM RENEWED SUCCESSOR NEXT GENERATION FOUR CONTINUITY'
_ESTABLISH_PHRASE = 'ESTABLISH AURON TELEGRAM SUCCESSOR NEXT GENERATION FIVE BASELINE'
...
class SuccessorBaselineRequest(BaseModel):
    ...
    successor_next_generation_five_hash: str = Field(min_length=64, max_length=64, pattern='^[0-9a-f]{64}$')
...
def reset_telegram_expired_renewed_successor_next_generation_four_restoration_store() -> None:
...
def admit_recertification(payload: RecertificationAdmissionRequest) -> dict:
    if payload.admission_phrase != _ADMIT_PHRASE:
        raise HTTPException(status_code=403, detail='Explicit expired renewed successor-next-generation-four recertification admission required')
    existing = _admission_store.get(payload.continuity_id)
    if existing is not None:
        return {'state': 'telegram-expired-renewed-successor-next-generation-four-recertification-already-admitted', 'admission': existing, 'idempotent_replay': True, 'external_calls_made': 0}
    ...
    if continuity is None or expiry is None:
        raise HTTPException(status_code=409, detail='Expired v21.387 baseline with immutable expiry evidence required')
    checks = {
        'continuity_expired': continuity.get('continuity_state') == 'renewed-successor-next-generation-four-baseline-expired-recertification-required',
        'expiry_state_valid': expiry.get('expiry_state') == 'renewed-successor-next-generation-four-baseline-expired',
    ...
    admission = {'admission_id': str(uuid4()), **data, 'admission_state': 'expired-renewed-successor-next-generation-four-recertification-admitted', 'integrity_hash': _hash(data), 'immutable': True, 'admitted_by': payload.actor, 'admitted_at': datetime.now(timezone.utc).isoformat(), 'external_calls_made': 0}
    _admission_store[payload.continuity_id] = admission
    return {'state': 'telegram-expired-renewed-successor-next-generation-four-recertification-admitted', 'admission': admission, 'external_calls_made': 0}
...
def restore_continuity(payload: ContinuityRestorationRequest) -> dict:
    if payload.restoration_phrase != _RESTORE_PHRASE:
        raise HTTPException(status_code=403, detail='Explicit renewed successor-next-generation-four continuity restoration required')
    existing = _restoration_store.get(payload.continuity_id)
    if existing is not None:
        return {'state': 'telegram-renewed-successor-next-generation-four-continuity-already-restored', 'restoration': existing, 'idempotent_replay': True, 'external_calls_made': 0}
    ...
    restoration = {'restoration_id': str(uuid4()), **data, 'restoration_state': 'renewed-successor-next-generation-four-continuity-restored-awaiting-successor-next-generation-five-baseline', 'integrity_hash': _hash(data), 'immutable': True, 'restored_by': payload.actor, 'restored_at': datetime.now(timezone.utc).isoformat(), 'external_calls_made': 0}
    _restoration_store[payload.continuity_id] = restoration
    continuity['continuity_state'] = restoration['restoration_state']
    return {'state': 'telegram-renewed-successor-next-generation-four-continuity-restored', 'restoration': restoration, 'continuity': continuity, 'external_calls_made': 0}
...
def establish_successor_baseline(payload: SuccessorBaselineRequest) -> dict:
    if payload.establishment_phrase != _ESTABLISH_PHRASE:
        raise HTTPException(status_code=403, detail='Explicit successor-next-generation-five baseline establishment required')
    existing = _succession_store.get(payload.continuity_id)
    if existing is not None:
        return {'state': 'telegram-successor-next-generation-five-baseline-already-established', 'succession': existing, 'idempotent_replay': True, 'external_calls_made': 0}
    ...
    checks = {
        ...
        'successor_hash_distinct': payload.successor_next_generation_five_hash != previous_hash,
    }
    blockers = [name for name, passed in checks.items() if not passed]
    if blockers:
        raise HTTPException(status_code=409, detail={'message': 'Successor baseline establishment blocked', 'blockers': blockers})
    now = datetime.now(timezone.utc)
    data = {'continuity_id': payload.continuity_id, 'restoration_id': restoration['restoration_id'], 'superseded_baseline_hash': previous_hash, 'successor_next_generation_five_hash': payload.successor_next_generation_five_hash, 'next_health_check_due_at': (now + timedelta(days=payload.health_check_interval_days)).isoformat(), 'valid_until': (now + timedelta(days=payload.validity_days)).isoformat(), 'succession_reference': payload.succession_reference, 'checks': checks}
    succession = {'succession_id': str(uuid4()), **data, 'succession_state': 'successor-next-generation-five-baseline-active', 'integrity_hash': _hash(data), 'immutable': True, 'established_by': payload.actor, 'established_at': now.isoformat(), 'external_calls_made': 0}
    _succession_store[payload.continuity_id] = succession
    continuity.update(continuity_state='successor-next-generation-five-baseline-active', active_baseline_hash=payload.successor_next_generation_five_hash, next_health_check_due_at=data['next_health_check_due_at'], valid_until=data['valid_until'])
    baseline = _baseline_store.get(continuity.get('monitoring_id'))
    if baseline is not None:
        baseline['baseline_state'] = 'renewed-successor-next-generation-four-baseline-superseded'
        baseline['superseded_by_succession_id'] = succession['succession_id']
    return {'state': 'telegram-successor-next-generation-five-baseline-established', 'succession': succession, 'continuity': continuity, 'superseded_baseline': baseline, 'external_calls_made': 0, 'next_layer': 'successor-next-generation-five-stabilization-observation-certification'}


@router.get('/status')
def status() -> dict:
    return {'admissions': len(_admission_store), 'restorations': len(_restoration_store), 'successions': len(_succession_store), 'external_calls_made': 0, 'mode': 'expired-renewed-successor-next-generation-four-restoration-succession-governance'}


@router.get('/command-center', response_class=HTMLResponse)
def command_center() -> str:
    return '<!doctype html><html><head><title>AURON v21.388</title></head><body><h1>AURON TELEGRAM EXPIRED RENEWED SUCCESSOR NEXT GENERATION FOUR RESTORATION COMMAND CENTER</h1><p>Recertification admission, continuity restoration and successor-next-generation-five baseline succession governance.</p><p>Safe mode: no Telegram API call, no provider execution, no outbound message.</p></body></html>'
