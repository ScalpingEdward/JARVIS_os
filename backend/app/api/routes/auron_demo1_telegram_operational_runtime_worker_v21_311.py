...
def execute_runtime_worker(
    payload: TelegramRuntimeWorkerRequest,
    transport: Callable[[str, dict, int], tuple[int, dict]] | None = None,
) -> dict:
    ...
    receipt_result = capture_live_provider_receipt(TelegramLiveReceiptRequest(
    ...
    record = {
        ...
        'provider_message_id': provider_message_id,
        'provider_error': provider_error,
        'provider_response_ok': bool(provider_payload.get('ok')),
        'started_at': started_at,
        'completed_at': datetime.now(timezone.utc).isoformat(),
        'receipt_id': receipt_result['receipt']['receipt_id'],
        'telegram_api_calls_made': 1,
        'outbound_messages_sent': 1 if accepted else 0,
        'external_calls_made': 1,
    }
    _worker_run_store[payload.execution_id] = record
    return {
        'state': 'telegram-runtime-worker-call-completed',
        'run': record,
        'next_layer': 'telegram-live-delivery-state-commit',
    }
...
def runtime_worker_status() -> dict:
    return {
        ...
        'bot_token_loaded': _telegram_token() is not None,
        'worker_runs': len(_worker_run_store),
        'successful_calls': sum(1 for item in _worker_run_store.values() if item['accepted']),
        'failed_calls': sum(1 for item in _worker_run_store.values() if not item['accepted']),
        'worker_mode': 'single-approved-live-provider-call',
    }
...
def command_center() -> str:
    ...
    html = html.replace('v21.310', 'v21.311')
    return html.replace(
