...
router = APIRouter(prefix='/auron/demo1/v21.270', tags=['auron-demo1-single-use-execution-token'])
...
def _validate_arm_receipt(item, payload: ExecutionTokenIssueRequest) -> list[str]:
    ...
    if receipt.get('armed') is not True:
        blockers.append('armed')
    if receipt.get('operator_confirmed') is not True:
        blockers.append('operator_confirmed')
    if receipt.get('safety_checks_passed') is not True:
        blockers.append('safety_checks_passed')
...
@router.get('/token-status/{approval_id}')
...
def issue_token(payload: ExecutionTokenIssueRequest) -> dict:
    ...
    _token_store[str(item.id)] = {
        ...
        'adapter': payload.arm_receipt.get('adapter'),
        'execution_domain': payload.arm_receipt.get('execution_domain'),
        'expires_at': expires_at,
        'used': False,
    }
    return {
        'state': 'single-use-token-issued',
        'approval_id': str(item.id),
        ...
        'execution_performed': False,
        'adapter_invoked': False,
        'next_gate': 'single-use-token-consume',
        'reply': 'Ein einmaliger Execution Token wurde ausgestellt. Es wurde noch keine Live-Aktion ausgefuehrt.',
    }
...
def consume_token(payload: ExecutionTokenConsumeRequest) -> dict:
    ...
    if record['used']:
        raise HTTPException(status_code=409, detail='Execution token already used')
    if record['expires_at'] <= datetime.now(timezone.utc):
        raise HTTPException(status_code=409, detail='Execution token expired')
    if (
        record['session_id'] != payload.session_id
        or record['workspace_id'] != payload.workspace_id
        or record['operator_id'] != payload.operator_id
    ):
        raise HTTPException(status_code=403, detail='Execution token scope mismatch')
    if record['digest'] != _digest(payload.token):
    ...
    return {
        'state': 'execution-token-consumed',
        'approval_id': str(item.id),
        'authorization_receipt': authorization_receipt,
        'execution_performed': False,
        'adapter_invoked': False,
        'next_gate': 'live-adapter-invocation',
        'reply': 'One-Shot-Autorisierung konsumiert. Die Live-Aktion selbst wurde noch nicht ausgefuehrt.',
    }
