...
class DocumentsCanaryControlError(RuntimeError):
    pass


@dataclass(frozen=True)
class DocumentsHealthEvidence:
    evidence_id: str
    provider_id: str
    adapter_id: str
...
class DocumentsCanaryHealthCommandCentre:
    """G12 provider health/drift certification plus governed Documents canary controls."""

    def __init__(self, db_path: str | Path, adapter: DocumentsReadonlyCanaryAdapter,
    ...
    def _init_schema(self):
        with self._connect() as c:
            c.executescript('''
                CREATE TABLE IF NOT EXISTS documents_canary_health_evidence(
                ...
                CREATE TABLE IF NOT EXISTS documents_canary_command_journal(
    ...
    def record_health(self, *, healthy: bool, observed_at: str | None=None) -> DocumentsHealthEvidence:
        d=self.adapter.descriptor(); ts=observed_at or self._now(); fp=self.fingerprint()
        eid='documents-health-'+self._hash({'provider':d.provider_id,'adapter':d.adapter_id,'fp':fp,'healthy':healthy,'at':ts})[:24]
        with self._connect() as c:
            c.execute('INSERT OR IGNORE INTO documents_canary_health_evidence VALUES (?,?,?,?,?,?)',
                (eid,d.provider_id,d.adapter_id,fp,int(healthy),ts))
        return DocumentsHealthEvidence(eid,d.provider_id,d.adapter_id,fp,healthy,ts)

    def certify_latest_health(self, *, now: str | None=None) -> dict:
        with self._connect() as c:
            row=c.execute('SELECT * FROM documents_canary_health_evidence ORDER BY observed_at DESC,evidence_id DESC LIMIT 1').fetchone()
        if row is None:
    ...
    def snapshot(self) -> dict:
        ...
        actions=self._rows(self.adapter.db_path,'SELECT provider_ref,vertical,provider_id,scope,action_key,payload_hash,state,created_at FROM documents_canary_actions ORDER BY created_at DESC,provider_ref')
        stops=self._rows(self.adapter.db_path,'SELECT * FROM documents_canary_stops ORDER BY stopped_at DESC,activation_id')
        alerts=[]
        ...
        return {
            'workspace':'documents-readonly-canary',
    ...
    def stop(self, activation_id: str, *, actor: str, reason: str='operator-stop') -> dict:
        if not actor.strip(): raise DocumentsCanaryControlError('actor required')
        self.adapter.stop_canary(activation_id=activation_id,reason=reason)
        return {'activation_id':activation_id,'state':'stopped','mutation_enabled':False,'production_transport_enabled':False}

    def record_command(self, command_text: str, *, actor: str) -> dict:
        command=command_text.strip(); operator=actor.strip()
        if not command or not operator: raise DocumentsCanaryControlError('command and actor required')
        with self._connect() as c:
            cur=c.execute('INSERT INTO documents_canary_command_journal(actor,command_text,state,created_at) VALUES (?,?,?,?)',
