...
from .models import ApprovalRequest, AuditRecord, OrchestrationCreate, OrchestrationListResponse, OrchestrationRecord, OrchestrationStatusResponse
...
@router.post("/v1/executive-missions/orchestrations", response_model=OrchestrationRecord, status_code=status.HTTP_201_CREATED)
...
def get_orchestration(orchestration_id: UUID, workspace_id: str = Query(min_length=1, max_length=100)) -> OrchestrationRecord:
    record = executive_mission_orchestration_service.get(orchestration_id, workspace_id)
    if record is None:
        raise HTTPException(status_code=404, detail="Orchestration not found")
    return record

@router.post("/v1/executive-missions/orchestrations/{orchestration_id}/analyze", response_model=OrchestrationRecord)
def analyze_orchestration(orchestration_id: UUID, workspace_id: str = Query(min_length=1, max_length=100), actor_id: str = Query(min_length=1, max_length=100)) -> OrchestrationRecord:
    try:
...
@router.post("/v1/executive-missions/orchestrations/{orchestration_id}/approval", response_model=OrchestrationRecord)
