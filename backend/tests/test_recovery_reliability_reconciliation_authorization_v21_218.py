import pytest
from app.schemas.recovery_reliability_reconciliation_authorization_v21_218 import ReconciliationAuthorizationRequest, RecoveryStep
from app.services.recovery_reliability_reconciliation_authorization_v21_218 import evaluate_recovery_authorization, reset_seen_sources_for_tests
...
def req(**kw):
    data=dict(source_id='ready-217-a', source_state='reconciliation-ready', source_human_approved=True, workspace_id='ws-1', baseline_id='base-a', baseline_version=11, baseline_digest='dig-11', affected_consumers=['c1','c2'], healthy_consumers=['c3'], recovery_steps=[step(1,'c1'),step(2,'c2')], blast_radius=0.4, residual_risk=0.2)
...
def test_partial_ordered_approval_is_staged():
    assert evaluate_recovery_authorization(req(recovery_steps=[step(1,'c1',True),step(2,'c2')]), authorize=True).state == 'staged'

def test_all_steps_approved_is_recovery_ready():
    assert evaluate_recovery_authorization(req(recovery_steps=[step(1,'c1',True),step(2,'c2',True)]), authorize=True).state == 'recovery-ready'

def test_out_of_order_approval_blocks():
    assert evaluate_recovery_authorization(req(recovery_steps=[step(1,'c1'),step(2,'c2',True)]), authorize=True).state == 'blocked'
...
def test_duplicate_affected_consumer_blocks():
    assert evaluate_recovery_authorization(req(affected_consumers=['c1','c1']), authorize=True).state == 'blocked'
