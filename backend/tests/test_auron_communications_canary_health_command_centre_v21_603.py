...
from app.communications.auron_communications_canary_health_command_centre_v21_603 import CommunicationsCanaryHealthCommandCentre
from app.core.auron_integration_readiness_v21_603 import get_integration_readiness


def build(tmp_path,max_age=300):
    adapter=CommunicationsDraftCanaryAdapter(tmp_path/'adapter.db')
    cc=CommunicationsCanaryHealthCommandCentre(tmp_path/'cc.db',adapter,tmp_path/'exec.db',tmp_path/'rec.db',max_age_seconds=max_age)
    return cc,adapter
...
def test_fresh_healthy_evidence_certifies_and_snapshot_is_safe(tmp_path):
    ...
    assert snap['outbound_send_enabled'] is False and snap['provider_write_enabled'] is False
...
def test_descriptor_drift_is_detected(tmp_path):
    ...
    adapter.descriptor=lambda: replace(original(),allowed_actions=original().allowed_actions+('send-message',))
...
def test_operator_stop_persists_and_commands_never_execute(tmp_path):
    cc,adapter=build(tmp_path); cc.record_health(healthy=True)
    stopped=cc.stop('activation-comms-1',actor='operator-1',reason='manual-stop')
    command=cc.record_command('send message now',actor='operator-1')
    assert stopped['state']=='stopped' and adapter.is_stopped('activation-comms-1') is True
    assert command['state']=='recorded-not-executed'
    snap=cc.snapshot(); assert any(a['kind']=='stop' for a in snap['alerts'])
    assert snap['outbound_send_enabled'] is False
...
def test_g16_readiness_advances_to_g17():
    r=get_integration_readiness(); assert r['roadmap_version']=='v21.603'
    assert r['next_item']=='G17-next-provider-vertical-selection'
