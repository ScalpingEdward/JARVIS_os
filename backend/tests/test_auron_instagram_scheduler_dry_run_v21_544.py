...
def stack(tmp_path: Path):
    ...
    registry.add_calendar_entry(
        content_id='c1', brand_id='brand', account_id='ig', content_type='reel', title='Reel'
    )
    ...
    lifecycle.transition('c1', 'draft')
    lifecycle.transition('c1', 'review')
    lifecycle.transition('c1', 'approved')

    provider_snapshot = ProviderHealthSnapshot(
        account_id='ig',
        provider_account_ref='provider-1',
        username='brand',
        token_state='healthy',
        permission_state='healthy',
        granted_permissions=('account.read',),
        provider_reachable=True,
        observed_at=datetime(2026, 8, 16, 17, 0, tzinfo=timezone.utc).isoformat(),
        external_calls_made=0,
    )
    health = MetaInstagramReadHealthAdapter(
        tmp_path / 'health.sqlite3',
        registry,
        InMemoryMetaInstagramReadSource({'provider-1': provider_snapshot}),
    )
    assert health.sync_and_verify('ig').state == 'verified-read-only'

    approvals = InstagramDraftPreviewApprovalPolicy(
        tmp_path / 'approval.sqlite3', registry, lifecycle, health
    )
    preview = approvals.generate_preview('c1', actor='operator')
    approvals.approve_for_publish(
        'c1', preview_id=preview.preview_id, approved_by='operator', reason='approved'
    )
    ...
    scheduler = InstagramSchedulerDryRun(
        tmp_path / 'scheduler.sqlite3', registry, lifecycle, approvals
    )
...
def test_schedule_is_deterministic_and_idempotent(tmp_path):
    ...
    first = scheduler.schedule('c1')
    second = scheduler.schedule('c1')
...
def test_due_queue_and_simulation_make_no_external_calls(tmp_path):
    ...
    result = scheduler.simulate(
        plan.plan_id, at=datetime(2026, 8, 17, 12, 1, tzinfo=timezone.utc)
    )
...
def test_not_due_cannot_simulate(tmp_path):
    ...
    with pytest.raises(ContentSchedulerError):
        scheduler.simulate(
            plan.plan_id, at=datetime(2026, 8, 17, 11, 59, tzinfo=timezone.utc)
        )
