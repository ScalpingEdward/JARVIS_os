...
def test_routes_registered() -> None:
    ...
    assert '/auron/demo1/v21.474/monitoring/start' in paths
    assert '/auron/demo1/v21.474/health/audit' in paths
    assert '/auron/demo1/v21.474/drift/open' in paths
    assert '/auron/demo1/v21.474/baseline/certify' in paths
    assert '/auron/demo1/v21.474/status' in paths
    assert '/auron/demo1/v21.474/command-center' in paths


def test_safe_empty_status() -> None:
    result = status()
    assert result['external_calls_made'] == 0
    assert result['mode'] == 'successor-next-generation-thirty-monitoring-drift-renewed-baseline'


def test_explicit_monitoring_phrase_enforced() -> None:
    response = client().post('/auron/demo1/v21.474/monitoring/start', json={
...
def test_explicit_audit_phrase_enforced() -> None:
    response = client().post('/auron/demo1/v21.474/health/audit', json={
...
def test_explicit_drift_phrase_enforced() -> None:
    response = client().post('/auron/demo1/v21.474/drift/open', json={
...
def test_explicit_baseline_phrase_enforced() -> None:
    response = client().post('/auron/demo1/v21.474/baseline/certify', json={
