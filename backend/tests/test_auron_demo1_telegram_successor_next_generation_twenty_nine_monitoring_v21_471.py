...
def test_routes_registered() -> None:
    ...
    assert '/auron/demo1/v21.471/monitoring/start' in paths
    assert '/auron/demo1/v21.471/health/audit' in paths
    assert '/auron/demo1/v21.471/drift/open' in paths
    assert '/auron/demo1/v21.471/baseline/certify' in paths
    assert '/auron/demo1/v21.471/status' in paths
    assert '/auron/demo1/v21.471/command-center' in paths


def test_safe_empty_status() -> None:
    result = status()
    assert result['external_calls_made'] == 0
    assert result['mode'] == 'successor-next-generation-twenty-nine-monitoring-drift-renewed-baseline'


def test_explicit_monitoring_phrase_enforced() -> None:
    response = client().post('/auron/demo1/v21.471/monitoring/start', json={
...
def test_explicit_audit_phrase_enforced() -> None:
    response = client().post('/auron/demo1/v21.471/health/audit', json={
...
def test_explicit_drift_phrase_enforced() -> None:
    response = client().post('/auron/demo1/v21.471/drift/open', json={
...
def test_explicit_baseline_certification_phrase_enforced() -> None:
    response = client().post('/auron/demo1/v21.471/baseline/certify', json={
