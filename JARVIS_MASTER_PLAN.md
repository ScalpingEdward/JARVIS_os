...
- Completed: D9 — Research provider/adapter onboarding contract with search/fetch/source-metadata/citation requirements and simulation-first safety; D10 — persistent normalized query/source/result registry with stable source identities, canonical URLs, content-integrity hashes, evidence hashes bound to source content, attribution, explicit freshness state and append-only evidence history; D11 — onboarding-certified provider read/search/fetch integration with provider source-reference persistence, search-to-fetch identity and URL verification, normalization into D10 and zero downstream actions; D12 — fail-closed evidence admission policy verifying query/result/source provenance, current-source evidence integrity, attribution and freshness, with transparent confidence scoring and minimum-confidence filtering; D13 — deterministic local report simulation using only D12-admissible evidence, explicit evidence-bound citation references, stable report IDs/hashes, persistent report state and zero downstream execution; D14 — persistent controlled research watches with bounded cadence, provider identity binding, explicit enablement, operator enablement and kill switch, executing only governed D11 reads plus D13 local report simulations with idempotent run slots and zero downstream actions; D15 — persistent watch-run reconciliation with post-run evidence freshness verification, stale-evidence fail-closed state and bounded exponential retry scheduling that re-enters the D14 policy boundary.
- Next after D15 merge: D16 — Research Command Centre operations exposing research queries, sources, evidence/freshness, watches, runs, reconciliation/retry state and governed watch controls while preserving the persistent command field.
