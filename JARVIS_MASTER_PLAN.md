...
- Next: build on the accounts registry — a state/compliance reconciliation loop that reads real broker snapshots (via the existing read-only MT5 bridge) into `POST /v1/accounts/{id}/state`, then, in a later gated PR, connect strategy assignments to the existing approval-gated executor modules. Continue extending the Research allowlist deliberately as real needs arise; consider real wiring for Instagram, Communications, Automation, and Documents verticals following the Research pattern (one real endpoint, explicit safety boundaries, audit log).
