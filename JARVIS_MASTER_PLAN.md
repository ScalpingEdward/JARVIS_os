...
- Completed: D1 — provider/adapter onboarding contract with simulation-first/read-only certification and outbound execution disabled; D2 — persistent provider-independent account/channel registry plus normalized conversation/message state, unread tracking, message integrity hashes and idempotent message insertion; D3 — onboarding-certified read-only provider synchronization with account/provider identity checks, normalized conversation/message ingestion and idempotent replay.
- Next after D3 merge: D4 — communications policy/approval boundary for drafts, replies and outbound intents. No provider send yet.
