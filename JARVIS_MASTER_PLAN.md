...
- Completed: D1 — provider/adapter onboarding contract with simulation-first/read-only certification and outbound execution disabled; D2 — persistent provider-independent account/channel registry plus normalized conversation/message state, unread tracking, message integrity hashes and idempotent message insertion; D3 — onboarding-certified read-only provider synchronization with account/provider identity checks, normalized conversation/message ingestion and idempotent replay; D4 — immutable content-hash-bound approval policy for new-message/reply intents with revocation and simulation-only authorization; D5 — deterministic dry-run plans bound to the current D4 approval and payload hash, with pre-simulation channel/reply/content revalidation and zero provider writes.
- Next after D5 merge: D6 — controlled communications execution boundary with explicit per-account/channel enablement, current D4 approval + successful D5 proof, kill-switch enforcement and disabled-by-default provider writer.
