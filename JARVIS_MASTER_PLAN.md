...
H1-H20 establish and certify the governed Research provider path through a one-shot bounded session and a zero-transport injection contract. H21 adds the design-only transport-injection activation boundary: one clean H20 certification is bound to its exact H19 contract/H18 session, an opaque `transportref://` reference and explicit operator identity. Read-only, operator re-approval, kill-switch, rollback, endpoint/capability and fail-closed budget controls are mandatory. Injection authorization, transport injection, credential resolution, networking, writes and production transport all remain disabled.
...
- H21 complete: the activation design pins one clean H20 certification to the exact H19/H18 identity plus an opaque transport reference and mandatory read-only/re-approval/kill/rollback controls. It does not authorize or inject transport and cannot execute network traffic.
- Next after H21 merge: H22 — Research real-provider transport injection activation design certification. Certify the H21 activation design against H20/H19/H18 evidence and verify opaque-reference, read-only, re-approval, kill/rollback, exact binding and zero-authorization/zero-transport state before any injection authorization gate.
