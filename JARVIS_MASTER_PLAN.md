...
H1-H16 establish the governed Research provider path through a zero-transport execution-boundary design. H17 certifies that design against the exact H15 canary token and verifies one-time token-consumption semantics, exact endpoint/capability binding, fail-closed 1..10 request-budget enforcement, audit-safety invariants and continued zero-transport state. H17 remains certification-only: it does not consume tokens, resolve credentials, configure transport or execute provider traffic.
...
- Next after H17 merge: H18 — Research real-provider one-shot canary execution gate. Introduce the first executable boundary that may consume one certified H15 token exactly once under the H17-certified endpoint/capability/request-budget contract, while concrete provider transport remains separately injected and fail-closed by default.
