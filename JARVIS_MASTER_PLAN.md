...
H1-H27 establish and certify the governed Research provider path through bounded activation, authorization and one persistent revocable transport identity binding. H28 defines the separately injectable transport-object boundary tied exactly to one clean H27-certified identity. The design pins provider/capability/HTTPS endpoint/GET/budget/timeout/response-size/opaque transport-ref scope, requires revocation-safe lifecycle and metadata/hash-only audit semantics, and remains object-absent, not injected and network-disabled.
...
- Next after H28 merge: H29 — Research real-provider transport object injection design certification. Certify the H28 object contract, exact H27/H26 identity binding, revocation-safe lifecycle, scope/budget/audit invariants and continued zero-object/zero-network state before any injection gate.
