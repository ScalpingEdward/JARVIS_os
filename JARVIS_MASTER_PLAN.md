...
- Completed: D9 — Research provider/adapter onboarding contract with search/fetch/source-metadata/citation requirements and simulation-first safety; D10 — persistent normalized query/source/result registry with stable source identities, canonical URLs, content-integrity hashes, evidence hashes bound to source content, attribution, explicit freshness state and append-only evidence history; D11 — onboarding-certified provider read/search/fetch integration with provider source-reference persistence, search-to-fetch identity and URL verification, normalization into D10 and zero downstream actions.
- Next after D11 merge: D12 — evidence/provenance/confidence policy for deciding which normalized research evidence is admissible for report assembly or later monitored workflows.
