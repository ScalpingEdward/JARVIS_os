...
- Completed: B1 — persistent multi-account registry plus provider/prop-firm rule-profile schema and conservative editable seed profiles; B2 — normalized persistent account state for balance, equity, P&L, exposure, positions, orders and trading-day state; B3 — persistent strategy/signal intake with validation, idempotency and strict separation from execution.
- Next after B3 merge: B4 — pre-trade risk engine with account-specific permitted risk, rule-headroom checks and fail-closed rejection before any execution.
