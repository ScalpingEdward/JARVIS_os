...
- Next after H37 merge: H38 — Research real-provider network execution gate. Consume one clean unexpired H37-certified authorization exactly once, reserve at most one bounded read-only request and keep actual provider traffic behind the same explicit fail-closed gate path.
