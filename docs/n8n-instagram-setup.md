...
1. **n8n's only job is fetching bytes -- AURON does the actual looking.** For each new photo (or a representative thumbnail frame for a video) in your Drive folder, n8n downloads it and calls `POST /v1/instagram/media-pool/analyze-and-ingest` with `{media_ref, media_type, image_base64 + image_media_type (or image_url), duration_seconds (video only)}`. AURON makes a real Claude vision call to actually look at the photo and derive a `theme` label (e.g. `"gold-trading-desk"`, `"mystic-symbol"`, `"quote-card"` — consistent labels are what let AURON group photos into a coherent carousel), `tags`, and a real `aesthetic_score` (0-1, judged from composition/lighting/coherence with the account's look, not guessed). Each item's outcome comes back individually, so one bad photo never blocks the rest of a 100-200-item batch. Video is analyzed via its thumbnail frame only for theme/tags/score -- Claude's vision reads a still image, not motion or audio, so treat a video's score/theme as a proxy from that one frame.

   **For a video that's too long for a Reel**, AURON can pick a real trim window too -- have n8n extract several frames at a regular interval across the whole clip (e.g. every 10-15s) and call `POST /v1/instagram/media-pool/{item_id}/analyze-trim` with `{frames: [{timestamp_seconds, image_base64/image_url}, ...], target_min_seconds, target_max_seconds}`. AURON looks at all the frames together and picks the strongest contiguous window -- but only from timestamps you actually sampled; it refuses (rather than guessing) if it can't confidently place the window within what it was shown. The result is stored on the pool item and flows automatically into the edit plan the next time `/curate` -> `/finalize` runs for it. Without this step, an over-length video is still flagged `trim_needed: true` in the edit plan, but `trim_start_seconds`/`trim_end_seconds` stay unset for a human to decide.
