...
from backend.app.modules.financial_close_nav.models import (
    CloseActionRequest,
    CloseState,
    FinancialCloseCreate,
    RiskDecision,
    ValuationPosition,
    ValuationStatus,
)
...
def act(service, record, action, **kwargs):
    return service.act(
        record.record_id,
        record.workspace_id,
        CloseActionRequest(action=action, actor="tester", **kwargs),
    )
...
def test_stale_prices_escalate():
    ...
    data = payload().model_copy(
        update={
            "positions": [
                payload().positions[0].model_copy(
                    update={"valuation_status": ValuationStatus.STALE}
                )
            ]
        }
    )
...
def test_risk_and_replay_controls():
    ...
    blocked = service.create(
        payload("blocked").model_copy(
            update={"source_key": "blocked", "risk_decision": RiskDecision.BLOCK}
        )
    )
...
def test_duplicate_positions_rejected():
    ...
    with pytest.raises(ValidationError):
        FinancialCloseCreate(
            **payload().model_dump()
            | {"positions": [position.model_dump(), position.model_dump()]}
        )
